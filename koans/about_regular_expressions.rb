...
class AboutRegularExpressions < EdgeCase::Koan
  ...
  def test_question_mark_means_optional
    ...
    # If part of the expression matches, then it returns the matching part
  ...
  def test_asterisk_means_zero_or_more
    ...
    # * means zero or more times. So it would never fail.
    # PROOF:
    assert_equal "", ""[/.*/]
  end
  ...
  #
  # Why?
  # Because they obtain as many characters as they can? Because they use the first match.
  ...
  def test_character_classes_give_options_for_a_character
    ...
    # Dont use horrible variable names, please
    assert_equal ["cat", "bat", "rat"], animals.select { |word| word[/[cbr]at/] }
  end
  ...
  def test_slash_w_is_a_shortcut_for_a_word_character_class
    ...
    # Only gets 1 match. The left most.
  ...
  def test_slash_a_anchors_to_the_start_of_the_string
    ...
    # \A makes it only match if its at the start of a string.
  ...
  # character class does not care about the order of the character (its a character class...)\
  # alternation only allows a specified "subexpression" between parenthesis.
  # ------------------------------------------------------------------
  ...
  def test_sub_is_like_find_and_replace
    ...
    # $1 is always the current match... $2 does not exist even if it is detected
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] }
  end

  def test_gsub_is_like_find_and_replace_all
    # - character is not a word character \w group
    assert_equal "one t-t t", "one two-three taz".gsub(/(t\w*)/) { $1[0, 1] }
  end
end
